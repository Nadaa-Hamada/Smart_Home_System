...
DataReceiver::DataReceiver(SerialPort &serial, int ledTemp, int ledLight, int irSensorPin)
    : serial(serial), ledTemp(ledTemp), ledLight(ledLight), irSensorPin(irSensorPin)
{
        ...
        if (n > 0)
        {
            ...
            if (tpos != string::npos && lpos != string::npos)
            {
                ...
                try
                {
                    ...
                    if (readGPIO(irSensorPin) == 0 && light < 150)
                    {
